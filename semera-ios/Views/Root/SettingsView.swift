...
struct SettingsView: View {
		...
		NavigationStack {
			ScrollView {
				VStack(alignment: .leading, spacing: 24) {
					...
					SettingsCard(title: "Appearance", caption: "Customize how Semera looks on your Home Screen.") {
	...
	private var privacyPolicyParagraphs: [String] {
		[
			"Semera is designed to keep chats and downloaded models on your device.",
			...
			"Downloaded models are stored locally so Semera can run them on-device. You can remove downloaded models from the model marketplace.",
			"Semera does not require an account for local chat. If future features use network services, this policy should be updated before those features ship."
		]
	}

	private var termsParagraphs: [String] {
		[
			"Semera provides local AI chat tools and model management features.",
